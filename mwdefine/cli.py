...
def main():
    parser = argparse.ArgumentParser(description="Query Merriam-Webster for a word definition.")
    parser.add_argument("word", help="Word to define")
    parser.add_argument("--api-key", default=os.getenv("MW_API_KEY"), help="Merriam-Webster API key (or set MW_API_KEY)")
    parser.add_argument("--raw", action="store_true", help="Show raw API response")
    args = parser.parse_args()

    if not args.api_key:
        print("API key required. Use --api-key or MW_API_KEY env variable.", file=sys.stderr)
        sys.exit(1)
